...
def test_folder_path(cifar10_dataset_folder_path):
    assert cifar10_dataset_folder_path is not None, 'Cifar-10 data folder no set.'
    assert cifar10_dataset_folder_path[-1] != '/', 'The "/" shouldn\'t be added to the end path.'
    assert os.path.exists(cifar10_dataset_folder_path), 'Path no found.'
...
def test_normalize(normalize):
    ...
    assert normalize_out.max() <= 1 and normalize_out.min() >= 0, 'Incorrect Range. {} to {} found'.format(normalize_out.min(), normalize_out.max())
...
def test_one_hot_encode(one_hot_encode):
    ...
    assert np.array_equal(enc_labels, new_enc_labels), \
        ...
        '{}\n' \
        'Make sure you save the map of labels to encodiings outside of the function.'.format(enc_labels, new_enc_labels)
